...
def call_list_alerts(http_client, start_time, end_time):
  # Construct the URL
  BACKSTORY_API_V1_URL = 'https://backstory.googleapis.com/v1'
  LIST_ALERTS_URL = '{}/alert/listalerts?start_time={}&end_time={}&page_size=10000'.format(BACKSTORY_API_V1_URL, start_time, end_time)
  # Make a request
  response = http_client.request(LIST_ALERTS_URL, 'GET')
  # Parse the response
  if response[0].status == 200:
    alerts = loads(response[1].decode('utf-8'))
...
if __name__ == "__main__":
  ...
  while True:
    current_time = time()
    # Call list alerts if the polling interval has been reached
    if current_time - last_poll > POLLING_INTERVAL:
      alerts = call_list_alerts(http_client, get_timestamp(last_poll), get_timestamp(current_time))
      post_to_humio(alerts)
